...
var bitly = new Bitly(config.bitly_secret);
...
module.exports = function(app) {
  ...
  var handleMenu = function(req, res) {
    ...
    if (_.keys(DIGIT_MAP).indexOf(numberPressed) === -1) {
      ...
      source.obj.get(source.key, country).then(function(message) {
        ...
        twiml.pause(1);
        twiml.say('An SMS containing more details has been sent to your phone. Thank you for calling, good bye.');
  ...
  var sendDataMessage = function(req, numberPressed) {
    ...
    var url = urls[country.iso3];

    twilioClient.sendMessage({
      from: req.query.To,
      to: req.query.Caller,
      body: "Check out Resource Watch for more " + source.name + " data " + url + " or learn about Resource Watch http://resourcewatch.org"
